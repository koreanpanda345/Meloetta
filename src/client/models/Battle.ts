...
export class Battle extends EventEmitter {
	...
	public addLine(command: string, sections: string[]) {
		...
		switch(command) {
			...
			case 'switch':
			case 'drag':
				this.emit('switchAndDrag', this.getDetails(sections[0]), [Number(sections[2].split('/')[0]), Number(sections[2].split('/')[1])]);
				break;
			case 'faint':
				this.emit('faint', this.getDetails(sections[0]));
				break;
			case 'move':
				this.emit('move', this.getDetails(sections[0]), (sections[2] === '' ? '': this.getDetails(sections[2])), sections[1]);
				break;
			
			// Minor Actions
			case '-damage':
				this.emit('-damage', this.getDetails(sections[0]), [Number(sections[1].split('/')[0]), Number(sections[1].split('/')[1])]);
				break;
			case '-start':
				this.emit('-start', this.getDetails(sections[0]), sections[1]);
				break;
			case '-heal':
				this.emit('-heal', this.getDetails(sections[0]), [Number(sections[1].split('/')[0]), Number(sections[1].split('/')[1])]);
				break;
			case 'upkeep':
				this.emit('upkeep');
				break;
